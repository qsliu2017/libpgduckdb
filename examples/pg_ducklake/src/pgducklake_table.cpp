...
DECLARE_PG_FUNCTION(ducklake_create_table_trigger) {
  ...
  if (IsA(parsetree, CreateTableAsStmt) && !pgducklake::ctas_skip_data) {
    ...
    // libpgduckdb's pgduckdb_get_querydef takes an optional DeparseRoutine
    // override; passing NULL yields the library default mapping.
    const char *ctas_query_string = pgduckdb_get_querydef(ctas_query, nullptr);
